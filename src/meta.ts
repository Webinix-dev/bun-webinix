...
import webinix_win_dll from "../libs/webinix-2.dll" with { type: "file" };
...
// lib_path
export const lib_path = join(appDataDir, "webinix-2.dll");

if (!existsSync(lib_path)) {
  debug("lib not exist, start extract lib");
  await write(lib_path, file(webinix_win_dll));
